    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        minAction, minDist,j = legal[0],999999,0
        while j < len(legal):
            for ghostPos in [i.argMax() for i in livingGhostPositionDistributions]:
                ghostDist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, legal[j]), ghostPos)
                minDist = min(minDist, ghostDist)
                if minDist==ghostDist:
                    minAction = legal[j]
            j+=1
        return minAction
